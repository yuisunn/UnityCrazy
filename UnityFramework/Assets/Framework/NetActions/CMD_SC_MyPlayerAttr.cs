...
namespace SPSGame
{
    public class CMD_SC_MyPlayerAttr : NetAction
    {
        ...
        public override ActionParam DecodePackage(NetPackage package)
        {
            ...
            MyPlayerAttr data = new MyPlayerAttr();
            if (0 >= package.DataReader.Msg2NetData<MyPlayerAttr>(data))
                return null;

            MyPlayer myplayer = Local.Instance.GetMyPlayer;
            if (null != myplayer)
                myplayer.OnRcvAttr(data);
            return ActParam;
        }
