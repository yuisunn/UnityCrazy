...
namespace SPSGame
{
    public class UnityAction_MoveSprite : UnityAction
    {
        public override bool ProcessAction()
        {
            try
            {
                ...
                EventManager.Trigger<EventMoveSprite>(new EventMoveSprite(spriteid, new Vector3(px / 10f, (float)py, pz / 10f), new Vector3(dx / 100f, dy / 100f, dz / 100f), speed/10f));
                return true;
            }
            catch (Exception ex)
            {
