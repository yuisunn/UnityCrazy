...
namespace SPSGame.Unity
{
    public class AssetBundleManager : Singleton<AssetBundleManager>
    {
        ...
        public void LoadMonsterByLoader(string filename, string sourcename, UnityAction<Object> cb, bool forceloadAsyn = false)
        {
            GameObject obj = new GameObject(sourcename);
            MonsterLoader ml = obj.AddComponent<MonsterLoader>();
            ml.LoadMonster(filename, sourcename, cb);
        }
        ...
        void LoadAsset(string path, string sourcename, UnityAction<Object> cb, bool forceloadAsyn = true)
        {
            if (mAssetBundleDic.ContainsKey(path))
            {
                ...
                if (forceloadAsyn)
                {
                    UnityMain.StartCoroutine(LoadSourceEnumerator(ab, sourcename, cb));
                }
                else
                {
            ...
            {
                UnityMain.StartCoroutine(LoadBundleEnumerator(path, sourcename, cb, forceloadAsyn));
            }
        }
        ...
        void LoadAsset(string path,  UnityAction<AssetBundle> cb, bool managercharge = true)
        {
            ...
            {
                UnityMain.StartCoroutine(LoadBundleEnumerator(path, cb, managercharge));
            }
        }
